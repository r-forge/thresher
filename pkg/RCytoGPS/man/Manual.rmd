...
## Description
 ...
 A function that converts conventional karyotypes from the standard text-based notation (the International Standard for Human Cytogenetic Nomenclature; ISCN) into a binary vector with three bits (loss, gain, or fusion) per cytoband, which we call the "LGF model".There are two supplementary function that readLGF uses called 'extractOneLGF' and 'rnames' to format the lgf model and to generate IDs.


## Usage

```r
readLGF(files = x, folder = y)
```
...
## Details
 ...
 readLGF takes the arguements of a specific file name from a specific folder. If the file name was not specified then it will take all JSON files from the specified folder. If the folder was not specified then it will look in the current working directory. The function will output a nested list having: Status of the chromosomes (a list of 'success' or 'fails'), a dataframe of the LGF model of the JSON file, and an IdioData dataframe formated for the image function.


## Value


 A nested list of the binary LGF model, a data frame of the LGF data with columns to include chromosome number, start and end locations of the chromosomes, chromosomal arm, cytogenetic bands, and the column sums of the loss, gains, and fusions. As well as a list of which karyotype had a 'success' or 'fail' cytogenetic event. In an event of a 'fail', it could suggestion how to fix the syntax of the inputted karyotype.
...
# `Idioformat`: Idioformat

## Description


 The purpose of this function is generate and format the cytoData based off of the LGF dataframe obtained from extractOneLGF. Idioformat takes the dataset from the LGF dataframe and column sums the data based off what cytogenetic event that is taking place. Idioformat also introduces new required data columns to enable the function image to generate a set of idiographs for visual comparisons and pattern recognitions. The new columns being concatinated to the dataframe are Chromosome, loc.start, loc.end, arm, and band and then the column sum of each cytogenetic event. To get the data for the required columns you have to run the Idioformat.rda file to store the new data as vectors that will be concatinated into the cytoData dataframe.
...
## Value
 ...
 Returns a dataframe of the binary LGF model column summed, as well as several other columns that will be used in the image function.
