...
We use the Umpire R package (version `packageVersion("Umpire")`) to generate more complex and realistic
synthetic data. We then compute the Euclidean distances between elements. Finally, we use the Mercator 
R package (version `packageVersion("Mercator")`) to visualize the data. We use the mclust R package
(version `r packageVersion("mclust")`) to compute the adjusted Rand index, a measure of cluster quality
...
up to represent a kind of cancer with five subtypes corresponding to recurrent sets of "hits", where
...
## Build a CancerModel with 5 subtypes
nBlocks <- 20    
...
## Simulate Data
Now we can take a random sample of 1000 elements from the distribution that we just defined.
```{r simData}
set.seed(111544)
trueN <- 300
...
eucdist <- dis
trueGroups <- labels
save(dataset, eucdist, trueGroups, file="spExample.rda")
```
...
The silhouette width plot in the upper left panel indicates that each of the six clusters contains
some poorly-classified elements with negative silhouette widths. Both the multidimensional scaling
...
```{r fig01, fig.cap = .tag(1, "Hierachical Clustering, with 5 clusters.")}
set.seed(1987)
...
mercViews(vis, "Hierarchical Clustering, Six Clusters")
```
...
ari.min
```
The adjusted rand index of the best SillyPutty clustering is 1, meaning that we  have
recovered the true cluster assignments present in the data. Note that even the worst result that
we obtained from SillyPutty has a better ARI (0.78) than either hierarchical clustering (0.53) or
PAM (0.23). 
...
```{r fig04, fig.cap = .tag(4, "Random SillyPutty clustering,  K = 5.")}
randSillyBin <- remapColors(vis, setClusters(vis, y2@MX))
mercViews(randSillyBin, main = "SillyPutty Cluster Types, K = 5", 
...
```{r fig05, fig.cap = "Hierarchical Clustering + SillyPutty, K = 5."}
hierSilly <- SillyPutty(vis@clusters, dis)
hierSillyBin <- remapColors(vis, setClusters(vis, hierSilly@cluster))
mercViews(hierSillyBin, main = "HClust + Silly, k = 5",tag = paste("C", 1:trueK, sep = ""))
...
```{r RangeSillyPutty}
# should add this to the package
RangeSillyPutty <- function(distobj, start,end, N = 100, verbose = FALSE, ...) {
  set.seed(110)
  res <- list()
  for (k in start:end){
    #k = i
...
overall value occurs when K = 5, which is the true number of clusters.

```{r fig06, fig.width=6, fig.height=5, fig.cap=.tag(6, "Best mean siilhouette width, by number of clusters, found with Silly Putty.")}
y <- suppressWarnings( RangeSillyPutty(dis, start = 2, end = 12) )
